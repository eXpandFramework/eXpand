...
Note when using DailyCalendar, it behaves on the same principals as, for example, WeeklyCalendar defines a set of days that are excluded every week. Likewise, DailyCalendar defines a set of times that are excluded every day. ",
    ...
    public class XpandDailyCalendar : XpandTriggerCalendar, IDailyCalendar{
        ...
        public XPCollection<XpandDateRange> DateRanges => GetCollection<XpandDateRange>("DateRanges");

        string ITriggerCalendar.CalendarTypeFullName => "Quartz.Impl.Calendar.DailyCalendar";

        [Tooltip("Indicates whether the time range represents an inverted time range ")]
        public bool InvertTimeRange{
            get { return _invertTimeRange; }
            set { SetPropertyValue("InvertTimeRange", ref _invertTimeRange, value); }
        }

        IList<IDateRange> IDailyCalendar.DateRanges => new ListConverter<IDateRange, XpandDateRange>(DateRanges);
    }
}