        ...
        public static void Register(this ICustomFunctionOperator customFunctionOperator) {
            if (!(XafTypesInfo.Instance is TypesInfoBuilder.TypesInfo)) {
                ...
                if (registeredItem != null && registeredItem != customFunctionOperator && InterfaceBuilder.RuntimeMode){
                    //2015-08-05 Kelvin: Block internal exception
                    System.Diagnostics.Trace.TraceError("Unable to register operator: " + customFunctionOperator.Name);
                    //throw new UserFriendlyException("Unable to register operator: " + customFunctionOperator.Name);

                    if (registeredItem != null){
                        CriteriaOperator.UnregisterCustomFunction(registeredItem);
                        registeredItem = null;
                    }
