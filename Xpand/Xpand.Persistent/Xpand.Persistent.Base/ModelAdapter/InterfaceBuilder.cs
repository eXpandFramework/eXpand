...
namespace Xpand.Persistent.Base.ModelAdapter{
    ...
    public class InterfaceBuilder{
        ...
        public static bool ExternalModelEditor{
            get{
                return
                    Process.GetCurrentProcess().ProcessName.IndexOf(".ExpressApp.ModelEditor", StringComparison.Ordinal) >
                    -1;
            }
        }
        ...
        public bool IsDevMachine{
            get{
                return
                    Environment.GetEnvironmentVariable("XpandDevMachine", EnvironmentVariableTarget.User).Change<bool>();
            }
        }
        ...
        private static void RaiseCompilerException(String source, CompilerResults compilerResults, string assemblyFile){
            ...
            throw new CompilerErrorException(compilerResults, source,
                "Assembly=" + assemblyFile + Environment.NewLine + errors);
        }
        ...
        string GetPropertyCode(DynamicModelPropertyInfo property, Func<DynamicModelPropertyInfo, bool> filter,
            ...
            var propertyCode = string.Format("    {0} {1} {{ get; {2} }}", GetPropertyTypeCode(property, interfaceName),
                property.Name, setMethod);
        ...
        string GetAttributesCode(DynamicModelPropertyInfo property){
            ...
            return codeList.Aggregate<string, string>(null,
                (current, attributeCode) => current + string.Format("   [{0}]{1}", attributeCode, Environment.NewLine));
        }
        ...
        string GetAttributeCode(object attribute, DynamicModelPropertyInfo info){
            ...
            if (attributeType == typeof(ModelValueCalculatorWrapperAttribute)){
                ...
                if (modelValueCalculatorAttribute.CalculatorType != null){
                    return string.Format(@"{0}(typeof({1}))", TypeToString(typeof(ModelValueCalculatorAttribute)),
                        TypeToString(modelValueCalculatorAttribute.CalculatorType));
                }
            ...
            if (attributeType == typeof(ModelReadOnlyAttribute)){
                ...
                return string.Format(@"{0}(typeof({1}))", TypeToString(typeof(ModelReadOnlyAttribute)),
                    TypeToString(readOnlyCalculatorType));
            }
        ...
        public static string GeneratedDisplayNameCode(string arg3){
            var interfaceBuilder = new InterfaceBuilder();
            return string.Format(@"[{0}(""{1}"")]", interfaceBuilder.TypeToString(typeof(ModelDisplayNameAttribute)),
                arg3);
        }
    }
        ...
        public static PropertyInfo[] GetPublicProperties(this Type type){
            if (type.IsInterface){
                ...
                while (queue.Count > 0){
                    ...
                    var typeProperties =
                        subType.GetProperties(BindingFlags.FlattenHierarchy | BindingFlags.Public |
