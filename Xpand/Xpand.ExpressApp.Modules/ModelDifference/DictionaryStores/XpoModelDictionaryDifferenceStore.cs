...
namespace Xpand.ExpressApp.ModelDifference.DictionaryStores
{
    ...
    public class XpoModelDictionaryDifferenceStore : XpoDictionaryDifferenceStore
    {
        ...
        public const string IgnoreUserModelDiffs = "IgnoreUserModelDiffs";
        ...
        internal static bool IsIgnoreUserModelDiffs()
        {
            bool ignoreUserDiffs = false;
            bool.TryParse(ConfigurationManager.AppSettings["IgnoreUserModelDiffs"], out ignoreUserDiffs);
            return ignoreUserDiffs;
        }
        ...
        Dictionary<string, ModelDifferenceObjectInfo> GetLoadedModelApplications(ModelApplicationBase model)
        {
            ...
            PersistentApplication application = ObjectSpace.FindObject<PersistentApplication>(new BinaryOperator("UniqueName", Application.GetType().FullName));
            if (application == null)
                return new Dictionary<string, ModelDifferenceObjectInfo>();
            else
            {
                System.Guid appID = application.Oid;
