...
namespace Xpand.ExpressApp.IO.Core{
    public static class XmlExtensions{
        ...
        public static IEnumerable<XElement> Properties(this IEnumerable<XElement> elements, NodeType nodeType){
            return
                elements.Descendants("Property")
                    .Where(xElement => xElement.GetAttributeValue("type") == nodeType.ToString().MakeFirstCharLower());
        }
        ...
        public static XElement Property(this XElement element, string name){
            return element.Properties().FirstOrDefault(xElement => xElement.GetAttributeValue("name") == name);
        }

        public static XElement Property(this IEnumerable<XElement> properties, string name){
            return properties.FirstOrDefault(element => element.GetAttributeValue("name") == name);
        }

        public static XElement ObjectProperty(this XElement element, string name){
            return element.Descendants("Property").FirstOrDefault(xElement => xElement.GetAttributeValue("name") == name);
        }
