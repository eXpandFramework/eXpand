...
namespace FeatureCenter.Base {
    public class DummyDataBuilder {
        ...
        public DummyDataBuilder(Session session) {
            _session = session;
        }
        ...
        public void CreateObjects()
        {
            ...
            {
                if (!typeInfo.IsPersistent|| _session.FindObject(typeInfo.Type,null) != null) continue;
        ...
        public void CreateObjects(ITypeInfo typeInfo) {
            ...
            {
                i++;
                var customer = (ICustomer)ReflectionHelper.CreateObject(typeInfo.Type, new object[] { _session });
                customer.Name = name;
        ...
        void CreateOrderLine(string[] articles, Random rand, IOrder order, Dictionary<string, float> artprice, ITypeInfo orderLineTypeInfo)
        {
            ...
            {
                var line = (IOrderLine)ReflectionHelper.CreateObject(orderLineTypeInfo.Type, new object[] { _session });
                line.Order = order;
        ...
        {
            var order = (IOrder)ReflectionHelper.CreateObject(ordersTypeInfo.Type, new object[] { _session });
