...
namespace eXpand.ExpressApp.WorldCreator.ClassTypeBuilder {
    public class PersistentClassInfoTypeBuilder : Builder<Type>, IAssemblyNameBuilder, ITypeDefineBuilder
    {
        ...
        private List<TypeInfo> createTypes(IEnumerable<TypeBuilderInfo> builders) {
            return builders.OrderBy(info => info.Order).Select(builder => new TypeInfo(builder.TypeBuilder.CreateType(), builder.IPersistentClassInfo)).ToList();
        }
        ...
        private static void createGetSetProperty(TypeBuilder type, string memberName,IEnumerable<IPersistentAttributeInfo> attributeInfos, Type memberInfoType) {
