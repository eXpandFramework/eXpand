...
namespace eXpand.ExpressApp.WorldCreator {
    public class XpoObjectMerger {
        ...
        {
            var collection = new XPCollection(unitOfWork, persistentTypesInfoType).Cast<IPersistentClassInfo>().Where(info => info.MergedObjectType !=
                                                                                                                              null).ToList();
            foreach (IPersistentClassInfo classInfo in collection){
                XPClassInfo xpClassInfo = getClassInfo(classInfo.Session, classInfo.AssemblyName + "." + classInfo.Name,persistentTypes);
