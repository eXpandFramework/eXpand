...
{
    internal class ActionStateRulesNodeParser : StateAttributesNodeParser
    {
        protected override StateRuleAttribute GetActivationRuleAttribute(DictionaryNode childNode,
                                                                              ViewType viewType,
                                                                              XafApplication xafApplication)
        {
            var dictionaryActivationNodeWrapper = new DictionaryActionStateNodeWrapper(childNode,
                                                                                            xafApplication.Modules[0].
                                                                                                ModuleManager.
                                                                                                ControllersManager);

            return new ActionStateRuleAttribute(dictionaryActivationNodeWrapper.Action.Id,
                                                     dictionaryActivationNodeWrapper.Nesting,
